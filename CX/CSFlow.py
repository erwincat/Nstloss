...
def CX_loss(T_features, I_features, distance=Distance.L2, nnsigma=float(1.0)):
    ...
    with tf.name_scope('CX'):
        ...
        # To:
        cs = cs_flow.cs_NHWC
        k_max_NC = tf.reduce_max(cs, axis=height_width_axis) #找到CXij集合中最大的CXij
        CS = tf.reduce_mean(k_max_NC, axis=[1]) #计算均值，CX(X,Y)，返回的是一个数组
        CX_as_loss = 1 - CS
        CX_loss = -tf.log(1 - CX_as_loss)
        CX_loss = tf.reduce_mean(CX_loss) #返回损失值，一个float32
        return CX_loss