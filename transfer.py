...
add_arg('--semantic-weight', default=5, type=float,      help='Global weight of semantics vs. features.')
    ...
    def create_using_dotP(I_features, T_features,T_map_features,I_map_features, sigma = float(1.0), b = float(1.0)):
        ...
        with tf.name_scope('CS'):
            ...
            with tf.name_scope('IFeatures'):
                ...
                tf.print("cs_NHWC:",tf.shape(cs_flow.cs_NHWC))
                return cs_flow

    def calc_relative_distances(self,raw_distances, axis=3):
        epsilon = 1e-5
        div = tf.math.reduce_min(raw_distances, axis=axis, keepdims=True)
